...
for k = 1: numdata_test
 ...
 a_test_data = wavread(filename);
 c = fix(length(a_test_data)/2);
 a_test_cut = a_test_data(c-127: c+128);
 a_test_rceps = real(ifft(log(abs(fft(a_test_cut)))));
 a_test_cep = a_test_rceps(2:11);

 a2a_dist = norm(a_train_cep - a_test_cep);
 a2i_dist = norm(i_train_cep - a_test_cep);
 a2u_dist = norm(u_train_cep - a_test_cep);
 a2e_dist = norm(e_train_cep - a_test_cep);
 a2o_dist = norm(o_train_cep - a_test_cep);
